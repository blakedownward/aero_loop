...
def quantize_to_12bit_ei_format(signal: np.ndarray) -> np.ndarray:
    ...
    Args:
        signal: int16 array from WAV file (range [-32768, 32767])
    
    Returns:
        int32 array with 12-bit quantized values (range [-2049, 2047])
    """
    # Normalize to float [-1, 1]
    signal_float = signal.astype(np.float32) / 32768.0
    # Quantize to 12-bit: multiply by 2048, round, clip
    # This produces values in 12-bit range stored in int32 container (as EI does)
...
def predict_file(
    ...
    # EI converts 16-bit WAV to 12-bit during ingestion, so we must match that
